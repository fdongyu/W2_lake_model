...
#                 ax.axhline(y=Z[j], color='k')
                if lyrs[i,j] == 0:  ## masked grid
                    sq = patches.Rectangle((X[i], Z[j]), seg_length[i], self.lyr_height[j], linewidth=0.1, fill=True, color='0.6')
                elif lyrs[i,j] != 0:  ## computational grid
...
#                ax.add_patch(sq)
    ...
    def VisBranch3(self, branchID=1):
        """
        functions to use X and Z data of this class
        """
        ## read Segment info
        self.seg_length = self.readSeg('length')
        self.lyr_height = self.readSeg('Height')
